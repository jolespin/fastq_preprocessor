**Changes:**
* [2026.7.27] - `strobealign_wrapper --bam` now outputs only mapped alignments (`samtools view -F 4`) instead of all reads; unmapped reads are excluded from the coordinate-sorted BAM. No effect on `--coverage`/`--depth` output since unmapped reads have no mapping position [issue/#54](https://github.com/jolespin/fastq_preprocessor/issues/54)
